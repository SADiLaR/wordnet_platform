...
class EditorViewTest(TestCase):
    ...
    def test_status_htmx_get(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertNotIn("show_warning", response.context)
    ...
    def test_status_htmx_post_complete_triggers_warning(self):
        with self.subTest("no words, no relations"):
            ...
            self.assertTrue(response.context["show_warning"])
            self.assertFalse(response.context["no_definition"])
            self.assertTrue(response.context["no_words"])
            self.assertTrue(response.context["no_relations"])
            self.synset_a.refresh_from_db()
        ...
        with self.subTest("missing usage example"):
            ...
            self.assertTrue(response.context["show_warning"])
            self.assertFalse(response.context["no_definition"])
            self.assertFalse(response.context["no_words"])
            self.assertEqual(len(response.context["missing_usage_examples"]), 1)
